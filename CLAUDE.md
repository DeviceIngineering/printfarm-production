...
## 📋 Версия 3.5.0 (2025-08-16) - Redux Data Persistence & TypeScript Fixes

### 🆕 Новые возможности v3.5.0:
- **🔄 Redux Data Persistence** - данные Точки сохраняются между переключениями вкладок
- **⚡ TypeScript Fixes** - исправлены все ошибки компиляции TS2304, TS2322
- **🎯 Redux Refactoring** - полный переход TochkaPage с local state на Redux store
- **📤 Redux Export Actions** - функции экспорта переведены на Redux actions вместо manual fetch
- **🔧 Loading States** - правильная интеграция состояний загрузки для всех операций
- **💾 Проблема решена** - пользователи больше не теряют загруженные Excel файлы при навигации
- **🏗️ Architecture Improvement** - создан отдельный API слой для модуля Tochka
- **✅ TypeScript Compliance** - все критические ошибки компиляции устранены

### 🔄 Migration from v3.3.4:
- Интеграция резервных остатков сохранена
- Production hotfix остается активным
- Все предыдущие возможности работают стабильно
